...
public class RequestServiceActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void sellServiceUpload() {
        ...
        mUserDatabase = FirebaseDatabase.getInstance().getReference().child("Request_Service").child(Objects.requireNonNull(mAuth.getCurrentUser()).getUid() + request_title);
        mStorageRef = FirebaseStorage.getInstance().getReference();

        ///////////////////////////////////////// Save sell service info to fire base db
        if (imageHoldUri != null) {

            StorageReference mChildStorage = mStorageRef.child("request_service_picture").child(Objects.requireNonNull(imageHoldUri.getLastPathSegment()));
//                                String profilePicUrl = imageHoldUri.getLastPathSegment();
            ...
            mChildStorage.putFile(imageHoldUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    ...
                    mUserDatabase.child("request_title").setValue(request_title);
                    mUserDatabase.child("request_description").setValue(request_description);
                    mUserDatabase.child("request_price").setValue(request_price);
                    mUserDatabase.child("request_time").setValue(request_time);
                    mUserDatabase.child("request_user_id").setValue(mAuth.getCurrentUser().getUid());
                    assert imageUrl != null;
                    mUserDatabase.child("request_image_url").setValue(imageUrl.toString());
